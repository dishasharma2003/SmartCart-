...
function Nav() {
    ...
    const handleSearch = async (data) => {
        const q = (data.trainNo || "").trim();
        const iso = data.trainDate || ""; 
        ...
        <div className='w-full h-[80px] flex items-center justify-between md:justify-center gap-[30px] px-[20px] fixed top-0 z-[9999] bg-[#fff9f6] overflow-visible'>
            ...
            {showSearch && userData.role == "user" && <div className='w-[90%] h-[70px]  bg-white shadow-xl rounded-lg items-center gap-[20px] flex fixed top-[80px] left-[5%] md:hidden'>
                ...
                <div className='w-[80%] flex items-center gap-[10px]'>
                    ...
                    <input type="text" placeholder='Enter Train No.' className='px-[10px] text-gray-700 outline-0 w-full' onChange={(e) => setQuery(e.target.value)} value={query} />
                    <input type="date" className="px-[10px] py-[8px] text-gray-700 outline-none border border-gray-300 rounded-lg focus:border-[#ff4d2d] cursor-pointer" />
                </div>
            </div>}



            <h1 className='text-3xl font-bold mb-2 text-[#ff4d2d]'>Smart Cart</h1>
            {userData.role == "user" && <div className='md:w-[60%] lg:w-[40%] h-[70px] bg-white shadow-xl rounded-lg items-center gap-[20px] hidden md:flex'>
                ...
                <div className='w-[80%] flex items-center gap-[10px]'>
                    <IoIosSearch size={25} className='text-[#ff4d2d]' />
                    <form action="" className='flex row' onSubmit={handleSubmit(handleSearch)}>
                        <input type="text" {...register("trainNo")} placeholder='Enter Train No.' className='px-[10px] text-gray-700 outline-0 w-full' onChange={(e) => setQuery(e.target.value)} value={query} />
                        <input type="date" {...register("trainDate")} className="px-[10px] py-[8px] text-gray-700 outline-none border border-gray-300 rounded-lg focus:border-[#ff4d2d] cursor-pointer" />
                    </form>
                </div>
            </div>}
